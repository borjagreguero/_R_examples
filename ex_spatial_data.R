...
## The class has become a "SpatialPointsDataFrame" which is a type of S4 object that requires handling slightly differently. 
# The str() output contains lots of @ symbols which denote a different slot. Typing cycle@data will extract the attribute data. The X and Y locational information can now be found in the coords slot. In addition bbox contains the bounding box coordinates and the pro4string contains the projection, or CRS (coordinate reference system) information. We have not specified this so it is empty at the moment. We therefore need to refer to the correct Proj4 string information. 
# These are loaded with the rgdal package and can simply be referred to with an ID. To see the available CRSs enter:
EPSG<- make_EPSG()
head(EPSG)
...
## From this point we can combine the data with other spatial information and also perform transformations on the data. 
## This is especially useful when exporting to other software. 
