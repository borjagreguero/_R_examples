...
# Finish the command
my_tree_four <- rpart(Survived ~ Pclass + Sex + Age + 
...
# in the randomForest package. You call the function in a similar way as rpart():

#       no class option, so turn into factors!!!
# First your provide the formula. There is no argument class here to inform the 
# function you're dealing with predicting a categorical variable, so you need to turn Survived into a factor with two levels: as.factor(Survived) ~ Pclass + Sex + Age
...
my_forest <- randomForest(as.factor(Survived) ~ Pclass + Sex + Age, 
                          data = train, 
                          importance = TRUE, ntree = 3)

my_forest <- randomForest(as.factor(Survived) ~ Pclass + Sex + Age + 
                SibSp + Parch + Ticket + Embarked + Title, 
                data = train, 
                importance = TRUE, ntree = 3)
...
my_solution <- data.frame(Survived = my_prediction)
...
# the accuracy plot shows how much worse the model would 
# perform without the included variables. So a high decrease (= high value x-axis) 
# links to a high predictive variable. The second plot is the Gini coefficient. The higher the variable scores here, 
